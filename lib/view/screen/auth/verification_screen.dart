import 'dart:async';
import 'dart:convert'; // استيراد للمساعدة في تنسيق النصوص
                          ...
                          authController.verificationCode.length == 4
                              ? !authController.isLoading
                                    ? CustomButton(
                                        ...
                                        onPressed: () {
                                          if (widget.fromSignUp) {
                                            // 🚀 طباعة بيانات إرسال التحقق للتصحيح
                                            debugPrint(
                                              "==================== 🚀 [VERIFY REQUEST] 🚀 ====================",
                                            );
                                            debugPrint(
                                              "المسار المستهدف: /api/v1/auth/verify-phone",
                                            );
                                            debugPrint(
                                              "رقم الهاتف (phone): $_number",
                                            );
                                            debugPrint(
                                              "الرمز المدخل (otp): ${authController.verificationCode}",
                                            );
                                            debugPrint(
                                              "التوكن الممرر (Header Token): ${widget.token}",
                                            );
                                            debugPrint(
                                              "===============================================================",
                                            );

                                            authController.verifyPhone(_number, widget.token).then((
                                              value,
                                            ) {
                                              // 📥 طباعة رد السيرفر للتصحيح
                                              debugPrint(
                                                "==================== 📥 [VERIFY RESPONSE] 📥 ====================",
                                              );
                                              debugPrint(
                                                "نجاح التحقق برمجياً: ${value.isSuccess}",
                                              );
                                              debugPrint(
                                                "الرسالة المستلمة: ${value.message}",
                                              );
                                              debugPrint(
                                                "================================================================",
                                              );
